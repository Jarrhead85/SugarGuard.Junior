    ...
    private string BuildPrompt(GigaChatRequest request)
    {
        if (!string.IsNullOrWhiteSpace(request.StructuredContextJson))
        {
            ...
            return $"""
                ...
                7. Если подходящего предмета в рюкзаке нет, прямо скажи: "в рюкзаке подходящего перекуса не вижу".
                8. Ответ: 2-4 коротких предложения, понятных ребёнку. Без списков, без длинных дисклеймеров.
                """;
        }
